...
class PpoWebSession:
    ...
    def loginmethod1(self):
        ...
        r = self.session.post("https://poke-nexus.com/")
        hiddencookies = self.__gethiddeninput(r.content)
        hiddencookies2 = self.__gethiddeninput(r.content, 4)
        logincookies = self.__cookies.copy()
        logincookies[hiddencookies[0]] = hiddencookies[1]
        logincookies[hiddencookies2[0]] = hiddencookies2[1]

        self.session.post("https://poke-nexus.com/forums/index.php?action=login2", data=logincookies)

        self.session.post("https://poke-nexus.com/forums/index.php?action=login2;sa=check;member=" +
                          self.__getuserid())
    ...
    def logout(self):
        ...
        """
        response = self.session.post("https://poke-nexus.com/forums/index.php")
        for link in BeautifulSoup(response.content, "html.parser", parse_only=SoupStrainer('a')):
            if link.has_attr('href'):
                if "https://poke-nexus.com/forums/index.php?action=logout;sesc=" in link["href"]:
    ...
    def __getuserid(self) -> str:
        ...
        :return: the user id
        """
        userid = self.session.get("https://poke-nexus.com/getUserInfo.php")
    ...
    def __gethiddeninput(self, pagecontent: bytes, n=3) -> Tuple[str, str]:
        """
        ...
        element: lxml.html.InputElement = tree.xpath(f"/html/body/div[8]/div/div/div[2]/div/div[1]/div/form/div[2]/input[{n}]")[0]
